...
export function buildHelpText(): string {
  const sections = [
    ...
    // No date/time attached on purpose (v1 scope, PLAN.md 17.26) — a plain
    // to-do list, distinct from ปฏิทิน above which always needs a date+time.
    "• เพิ่มสิ่งที่ต้องทำ <ข้อความ> เช่น \"เพิ่มสิ่งที่ต้องทำ ซื้อของเข้าบ้าน\"",
  ...
  return ["ฉันช่วยได้ 10 เรื่อง — พิมพ์คำสั่งด้านล่าง หรือแตะเมนูใต้ช่องพิมพ์ก็ได้:", "", ...sections].join("\n");
}
